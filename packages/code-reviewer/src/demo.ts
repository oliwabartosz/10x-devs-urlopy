...
/**
 * Manual `npm run demo` sanity check for the code reviewer.
 *
  ...
  if (!process.env.OPENROUTER_API_KEY) {
    console.error(
      "Missing OPENROUTER_API_KEY. Copy .env.example to .env and add your key,\n" +
        "then run with: OPENROUTER_API_KEY=... npm run demo   (or use a dotenv loader).",
    );
    process.exitCode = 1;
    return;
  }
...
// Run the demo only when executed directly (e.g. `npm run demo`), not when imported.
