...
echo -n "$NEXT_CMD" | wl-copy 2>/dev/null || echo -n "$NEXT_CMD" | pbcopy 2>/dev/null || echo -n "$NEXT_CMD" | clip.exe 2>/dev/null || echo -n "$NEXT_CMD" | xclip -selection clipboard 2>/dev/null || true
```
...
If no clipboard tool is available (`wl-copy`, `pbcopy`, `clip.exe`, `xclip`, `Set-Clipboard`), drop the `(✓ copied to clipboard)` annotation but still print the suggestion.
