...
> **Clipboard convention.** Wherever this skill says *copy `X` to the clipboard*, pipe the exact string `X` to the platform clipboard — try `wl-copy` (Linux/Wayland), then `pbcopy` (macOS), then `clip.exe` (Windows/WSL), then `xclip -selection clipboard` (Linux/X11), and fall back silently if none exist. Then display the copied command on its own line suffixed with `(✓ copied)`.
...
8. **Capture and write back the SHA.** Run `git rev-parse --short HEAD` to get the `SHA`. For every Progress row flipped this phase, modify the file to change `- [x] N.M <title>` → `- [x] N.M <title> — <SHA>` (skip rows that already carry a SHA; if `SHA=""`, skip — `/10x-archive` surfaces SHA-less rows as informational warnings).
