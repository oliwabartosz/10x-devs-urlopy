...
#### 4. Bound the test suite's pooler client budget — *added during implementation*

**Files**: `vitest.config.ts`, `src/tests/helpers/db.ts`

**Intent**: Not foreseen when this phase was written; forced by adding a fourth route-level suite. Supabase's session pooler allows 15 clients, every suite holds a test pool, and every route-handler invocation opens another that is never `.end()`ed. Running suite files in parallel exhausted the budget partway through and later requests came back 503 `"Database error"` (`(EMAXCONNSESSION) max clients reached`) — indistinguishable from a real route defect, and dependent on how many suites happen to exist.

**Contract**: `fileParallelism: false` in `vitest.config.ts`, and `max=1&idle_timeout=1` appended to the connection string in `getTestDb()`, mirroring what `src/tests/helpers/astro-env.ts` already does for the route-handler pools. Recorded here because `fileParallelism: false` is a **project-wide test-runner behaviour change**, not a change local to this phase: every suite in the repo now runs serially. Measured cost at review time: ~150 s wall-clock for the full suite. These are network-bound tests, so that is small next to the false failures it removes.
...
`PATCH` — body `{ email: z.email() }`; malformed JSON → 400 `"Invalid JSON body"`; schema failure → 400 with `parsed.error.issues[0]?.message`. Reject a `deleted_at !== null` target → 409 `"Cannot update a deactivated employee"`, matching `[id].ts:96-98`. Call `adminClient.auth.admin.updateUserById(user_id, { email, email_confirm: true })`. **`email_confirm: true` is load-bearing** — omitting it leaves `email_verified: false`, which with confirmations enabled locks the worker out; this mirrors `createUser({ email_confirm: true })` at `employees/index.ts:128`. Duplicate detection is a **pre-check before the write**, not a mapping after it — **corrected on evidence during 3.13**. The 422 mapping this plan originally specified belongs to `createUser`; `updateUserById` returns `{ status: 500, code: "unexpected_failure", message: "Error updating user" }` for a duplicate address, byte-identical to any other server-side failure (verified against the live project). Mapping `unexpected_failure` to "duplicate" would label unrelated failures as collisions — the fragile matching `impl-review-phases-2-4.md` F4 warns against. So: `select 1 from auth.users where lower(email) = lower($1) and id <> $2` over the service-role connection, returning 409 `"Konto z tym adresem email już istnieje."` (the copy from `employees/index.ts:137-140` is still reused). The `authError.status === 422` branch is retained as a fallback in case Supabase ever starts returning it. A concurrent insert between the check and the write still lands as the opaque 500 — an accepted race requiring two moderators claiming one address in the same instant. Any other auth error → 500 `"Failed to update auth user"`. Success → 200 `{ email }`. Wrap failures in `Sentry.captureException` with `tags: { route: "PATCH /api/employees/:id/email" }`.
...
- [x] 4.5 E2E suite still passes: `npx playwright test` (the `getByLabel("Hasło", { exact: true })` locator still resolves uniquely) — 6/6 green, but note this suite targets the *deployed* app (`playwright.config.ts:23`), so it must be re-run after deploying Phase 4 for the locator check to actually exercise the new dialogs. **Re-run during the implementation review (2026-08-21) against the deployed Phase 4 build: 8/8 green** — the suite grew by two specs since the original run, and `auth.setup.ts`'s `getByLabel("Hasło", { exact: true })` still resolves uniquely with all four new dialogs live. The locator was never actually at risk: it runs on the signin page, which the new dashboard dialogs cannot reach — a14abe3
